...
library LAventities {
  ...
  {
    uint lobbyingPeriod = LAventitiesStorage.getLobbyingPeriod(_storage);
    uint votingPeriod = LAventitiesStorage.getVotingPeriod(_storage);
    uint revealingPeriod = LAventitiesStorage.getRevealingPeriod(_storage);

    uint deposit = getExistingAventityDeposit(_storage, _aventityId);
    challengeProposalId_ = LProposals.createProposal(_storage, deposit, lobbyingPeriod, votingPeriod, revealingPeriod);
  ...
  function aventityIsActiveAndNotUnderChallenge(IAventusStorage _storage, uint _aventityId)
    public
    view
    returns (bool aventityOk_)
  {
    aventityOk_ = aventityIsActive(_storage, _aventityId) && aventityIsNotUnderChallenge(_storage, _aventityId);
  }
