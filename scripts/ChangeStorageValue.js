const EventsManager = require('../AventusClassicJS/EventsManager.js');
...
async function main() {
  ...
  const value = (new BN(20)).mul(new BN(10).pow(new BN(18)));

  params.gas = await aventusApi.aventusStorage.methods.setUInt(key, value.toString()).estimateGas(params);
  ...
  console.log('Setting key:', key, 'to value:', value.toString(), '...');
  try {
    console.log('Before:', await aventusApi.aventusStorage.methods.getUInt(key).call());
    await aventusApi.aventusStorage.methods.setUInt(key, value.toString()).send(params);
